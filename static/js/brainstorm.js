...
// === 🧠 Запуск мозгового штурма из панели ===
// === 🧠 Запуск мозгового штурма из панели ===
async function runBrainstormFromOffice(folder, topic) {
  ...
  if (!progress || !output) {
    console.warn("⚠️ Элементы мозгового штурма не найдены.");
    return;
  }

  output.style.display = "block";
  output.classList.add("markdown-body");
  output.innerHTML = `🧠 <b>Мозговой штурм в каталоге "${folder}"</b><br>Тема: <i>${topic}</i><br><br>`;
  ...
  if (total === 0) {
    output.innerHTML = "⚠️ В каталоге нет сотрудников для штурма.";
