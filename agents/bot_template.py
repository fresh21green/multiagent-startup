...
# менеджер подставит сюда токен или оставит ENV fallback
__TELEGRAM_TOKEN_PLACEHOLDER__

TELEGRAM_API_URL = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}"
PROMT = __PROMPT_PLACEHOLDER__
...
def process_llm_response(chat_id: int, response: str):
    ...
    for match in pattern.finditer(response):
        ...
        if match.group(1):
            ...
            send_photo(chat_id, match.group(2))
        elif match.group(3):
            try:
                header, encoded = match.group(3).split(",", 1)
                image_bytes = base64.b64decode(encoded)
                send_photo_file(chat_id, image_bytes)
